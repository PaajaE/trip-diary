...
export function SyncStatusControl() {
  ...
  async function handleSync() {
    ...
    setSyncing(true)
    try {
      if (status === 'syncing' && user !== null) {
        await forceResetSyncingOperations(user.id)
      }
  ...
  return (
    <div className="relative">
      ...
      {open ? (
        <div
          className="absolute right-0 top-[calc(100%+0.5rem)] z-30 w-[min(20rem,calc(100vw-2.5rem))] rounded-xl border border-border bg-background p-4 shadow-soft"
          role="dialog"
        >
