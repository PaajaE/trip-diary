...
begin
  ...
  where id = p_entry_id and creator_id = v_user_id for update;
  if not found then
    raise exception using errcode = '42501', message = 'entry is unavailable or not owned by the current user';
  end if;
  ...
  if v_entry.space_id is distinct from (select space_id from public.journeys where id = p_journey_id) then
  ...
  if p_stage_id is not null and not exists (
    select 1 from public.journey_stages where id = p_stage_id and journey_id = p_journey_id
  ) then
  ...
  if v_has_location and (not p_latitude between -90 and 90 or not p_longitude between -180 and 180) then
  ...
  if v_stop_id is not null then
    select * into v_existing_stop from public.journey_stops where id = v_stop_id for update;
    if found then
      ...
      update public.journey_stops set
        stage_id = p_stage_id,
        status = 'visited',
      ...
      v_stop_title := coalesce(nullif(btrim(p_location_title), ''), nullif(btrim(v_entry.title), ''), 'Moment');
      insert into public.journey_stops (
        id, journey_id, stage_id, creator_id, title, status, position,
        latitude, longitude, map_latitude, map_longitude, visited_at
      )
      select
        v_stop_id, p_journey_id, p_stage_id, v_user_id, v_stop_title, 'visited',
        coalesce(max(position) + 1, 0), p_latitude, p_longitude,
  ...
  insert into public.entry_journey_links (entry_id, journey_id, stage_id, stop_id, creator_id)
  values (p_entry_id, p_journey_id, p_stage_id, v_stop_id, v_user_id)
