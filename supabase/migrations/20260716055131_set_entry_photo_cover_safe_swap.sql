-- Harden set_entry_photo_cover against non-deferrable unique index races.
...
begin
  ...
  -- Two updates: clear then set. A single CASE update can trip the
  -- non-deferrable partial unique index when the new cover becomes true
  -- before the previous cover is cleared.
